...
reliable access to NumPy files.  It does not require \proglang{Python} as it
relies on the CNPy library which is connected to \proglang{R} with the help
...
can consider an alternative which does not require CNPy--but which requires
...
to read NumPy data via \pkg{reticulate}.  
...
vignette, but using \pkg{reticulate} instead of CNPy.
...
```{r ex3}
gz <- import("gzip")  # for compressed data  
